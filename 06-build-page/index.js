...
async function removeFolder(where) {
  console.log('removeFolder', where);
  // if(where){
  //   if(where.isDirectory()){console.log('isDirectory');}
  //   if(where.isFile()){console.log('isFile');}
  return fs.promises
...
async function init() {
  removeFolder(projectDist)
    .then(() => {
      createFolder(path.join(projectDist, 'assets'));
      createFolder(projectDist);
      createFile(projectDist, 'style.css');
      fs.promises.copyFile(path.join(__dirname, 'template.html'),
        path.join(projectDist, 'index.html')).then(
        () => {
          makeCssBundle();
          copyFiles(
